...
public class PageMaker {
	...
	public void calcData() {
		...
		// 2.1) currPage가 속한 페이지블럭의 ePageNo를 계산
		// => n개씩 출력한다면 ePagNo는 늘 n의 배수
		// 	  displayPageNo=3 이면 3, 6, 9, 12, ......
		// => ex) 17 page 요청 , displayPageNo=3, 일 때 17이 몇번째 그룹 인지 계산하려면,
		...
		// => 계산의 결과로 얻어진 ePageNo가 실제 lastPageNo보다 크면 수정
		lastPageNo = (int)Math.ceil(totalRowCount/(double)cri.getRowsPerPage());
		// 예) 11/3 = 3, 11/(double)3 = 3.33333
	...
	// ** ver02
	// => uri에 search 기능 추가
	// => ?currPage=1&rowPerPage=3&searchType=t&keyword=Java
